...
public class takeDamageRusher : MonoBehaviour {
	...
	private rusherEnemyScript enemy;
	...
	void Start () {
		...
		enemy = this.GetComponent<rusherEnemyScript> ();
	...
	void OnTriggerEnter2D(Collider2D other)
	{
		if (other.tag == "Sword") {
			score.timerScore = 0;
			add_mult();
			score.tmpScore = score.tmpScore + 3;rigidbody2D.AddForce(new Vector2(0, y));
		...
		if (other.tag == "Spell") {
			score.timerScore = 0;
			add_mult();
			score.tmpScore = score.tmpScore + 5;rigidbody2D.AddForce(new Vector2(0, y));
			rigidbody2D.AddForce(new Vector2(-enemy.lookDirection*x, 0));
			DamageSpell();
		}	
